...
if (confirm('Почати тестування?')) {
  type SaleStatus = 'available' | 'unavailable'
  type Status = 'active' | 'inactive'
  interface IShortProductData{
    id:number;
    price:number;
    old_price:number;
    usd_price:string;
  }
  interface IShowInSite{
    id:number;
  ...
  interface IProductData{
    ...
    show_in_site:null | IShowInSite;
    status:Status;
    usd_price:string;
  }
  let dataList:IProductData[] = [
  {
    id: 344277463,
    old_price: 1395,
    old_usd_price: '37.70',
...
//* та ціну у доларах (usd_price)
    function getCopyArrayWithShortProductData(dataList:IProductData[]):IShortProductData[]{
