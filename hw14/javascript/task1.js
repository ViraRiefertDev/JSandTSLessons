...
if (confirm('Почати тестування?')) {
    ...
    function getSumOfSponsoredInvestment(sponsors) {
    ...
    const sponsors = companiesArray.flatMap((company) => company.sponsors);
    ...
    const copyCompaniesWithProfitLessThen10000 = JSON.parse(JSON.stringify(filteredArray));
    filteredArray = companiesArray.filter((company) => company.websiteValue >= 10000);
    const copyCompaniesWithProfitMoreThen10000 = JSON.parse(JSON.stringify(filteredArray));
    document.write(`
