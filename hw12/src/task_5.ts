...
if (confirm('Почати тестування?')) {
  ...
  "Mia"
     ...
     */
    function swap(arr:string[], index:number, prevIndex:number):void {
   ...
   */
    function bubblePassLeftToRight(arr:string[], rightIndexLimit:number):[number,boolean] {
        let changed = false;
     ...
     */
    function sortArrayBubble(array:string[]):string[] {
    ...
    /**
      * Виконує бінарний пошук у відсортованому масиві рядків.
      * Повертає індекс елемента, якщо він знайдений, або -1, якщо елемент відсутній.
      * Алгоритм працює тільки з масивом, попередньо відсортованим за зростанням (лексикографічно).
      * 
      * @param {string[]} arr - Відсортований масив рядків, у якому здійснюється пошук.
      * @param {string} searchElement - Значення, яке потрібно знайти.
      * @returns {number} Індекс знайденого елемента в масиві або -1, якщо елемент не знайдений.
     */
    function findIndex__binarySearch(arr:string[],searchElement:string):number{
      let start = 0
    ...
    if(searchIndex !==-1) document.write(`Olga входить у масив під індексом: ${searchIndex}`)
