...
if (confirm('Почати тестування?')) {
  ...
  */
    ...
    /**
       ...
       *
    */
    function findIndex__binarySearch(arr, lengthOfEl) {
        let start = 0;
        let end = arr.length - 1;
        while (start <= end) {
            const middle = Math.floor((start + end) / 2);
            if (arr[middle].length === lengthOfEl)
                return middle;
            if (arr[middle].length < lengthOfEl)
                start = middle + 1;
            if (arr[middle].length > lengthOfEl)
    ...
    if (searchIndex !== -1)
        document.write(`Перше знайдене ім'я з довжиною 5 символів ${sortedArrayInsertion[searchIndex]}, під індексом: ${searchIndex}`);
    else {
        document.write('У масиві не має імені з довжиною в 5 символів');
    }
}
